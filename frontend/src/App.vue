...
import { Cable, Database, LayoutDashboard, RefreshCw, Save, ShieldCheck, ListTodo, MessagesSquare } from 'lucide-vue-next'
...
<template>
  <div class="shell">
    <aside><div class="brand"><LayoutDashboard :size="22"/><strong>我的工作台</strong></div><button class="nav" :class="{active:tab==='archive'}" @click="tab='archive'"><MessagesSquare :size="18"/>聊天归档</button><button class="nav" :class="{active:tab==='sync'}" @click="tab='sync'"><ListTodo :size="18"/>同步管理</button><button class="nav" :class="{active:tab==='settings'}" @click="tab='settings'"><Cable :size="18"/>连接设置</button><div class="local"><ShieldCheck :size="16"/>本机工作空间</div></aside>
    <main>
      <header><div><span class="breadcrumb">工作台 / {{tab==='settings' ? '系统设置' : '数据归档'}}</span><h1>{{tab==='sync' ? '同步管理' : tab==='archive' ? '聊天归档' : '连接设置'}}</h1></div><el-tag type="info" effect="plain">阶段 3 · 待审核</el-tag></header>
      ...
      <ArchiveBrowser v-if="tab==='archive'"/><SyncManager v-else-if="tab==='sync'"/>
      ...
      </template><footer><ShieldCheck :size="15"/>令牌不落盘 <span>·</span> 数据库位置：{{db.path || '—'}}</footer>
    </main>
  </div>
</template>
